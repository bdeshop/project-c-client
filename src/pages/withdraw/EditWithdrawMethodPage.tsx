...
import {
  useWithdrawMethod,
  useUpdateWithdrawMethod,
  UserInput,
} from "../../lib/queries";
import { toast } from "sonner";
import { Plus, Trash2, Upload, Save, ArrowLeft, X } from "lucide-react";
import { API_URL } from "../../lib/api";

export default function EditWithdrawMethodPage() {
  ...
  const [existingMethodImage, setExistingMethodImage] = useState<string>("");
  const [existingWithdrawalPageImage, setExistingWithdrawalPageImage] =
    useState<string>("");

  // Load existing data
  useEffect(() => {
    if (withdrawMethodData?.data?.data) {
      const method = withdrawMethodData.data.data;
      setFormData({
        method_name_en: method.method_name_en || "",
        method_name_bd: method.method_name_bd || "",
        min_withdrawal: method.min_withdrawal?.toString() || "",
        max_withdrawal: method.max_withdrawal?.toString() || "",
        processing_time: method.processing_time || "",
        withdrawal_fee: method.withdrawal_fee?.toString() || "",
        fee_type: method.fee_type || "fixed",
        text_color: method.text_color || "#000000",
        background_color: method.background_color || "#E2136E",
        button_color: method.button_color || "#E2136E",
        instruction_en: method.instruction_en || "",
        instruction_bd: method.instruction_bd || "",
        status: method.status || "Active",
      });

      setUserInputs(method.user_inputs || []);

      if (method.method_image) {
        setExistingMethodImage(`${API_URL}/${method.method_image}`);
      }
      if (method.withdrawal_page_image) {
        setExistingWithdrawalPageImage(
          `${API_URL}/${method.withdrawal_page_image}`
        );
      }
    }
  }, [withdrawMethodData]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateWithdrawMethod.mutateAsync({ id: id!, data: formDataToSend });
      toast.success("Withdraw method updated successfully!");
      navigate(`/dashboard/withdraw/view/${id}`);
    } catch (error) {
      console.error("Withdraw method update error:", error);
      const errorMessage =
        (error as { response?: { data?: { message?: string } } })?.response
          ?.data?.message || "Failed to update withdraw method";
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    {existingMethodImage && !methodImage && (
                      <div className="relative">
                        <img
                          src={existingMethodImage}
                          alt="Current method"
                  ...
                  <div className="space-y-2">
                    ...
                    {existingWithdrawalPageImage && !withdrawalPageImage && (
                      <div className="relative">
                        <img
                          src={existingWithdrawalPageImage}
                          alt="Current withdrawal page"
                          className="w-full h-32 object-cover rounded border mb-2"
                        />
                        <p className="text-sm text-muted-foreground mb-2">
                          Current image
              ...
              <div className="flex justify-end gap-4 pt-6 border-t border-border">
                ...
                <Button
                  type="submit"
                  disabled={updateWithdrawMethod.isPending}
                  className="gradient-primary"
                >
                  <Save className="h-4 w-4 mr-2" />
                  {updateWithdrawMethod.isPending
                    ? "Updating..."
